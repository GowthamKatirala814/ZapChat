...
public class SecurityHeadersMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        context.Response.OnStarting(() =>
        {
            ...
            // Content Security Policy — allow resources from same origin and localhost services
            headers["Content-Security-Policy"] =
                "default-src 'self'; " +
                "connect-src 'self' https://localhost:5000 ws://localhost:5000 wss://localhost:5000; " +
